...
for dir_mode = 1:2
    ...
    for i = 1:2
        ...
        for m = 1:n_mics
            ...
            for idx = 1:length(harmonics)
                ...
                p_rms_bpf(idx) = 2*abs(C_j)/sqrt(2);
            end
    ...
    if theta_dir == true
        ...
        title(ax, '\textbf{$\theta$ Directivity}', 'Interpreter', 'latex', 'FontSize', 14);
    else
        ax.ThetaDir = 'counterclockwise';
        ax.ThetaZeroLocation = 'top';    
        polarplot(ax, 0, r_center, 'ko', 'MarkerFaceColor', 'k', 'MarkerSize', 6, 'HandleVisibility', 'off');
        title(ax, '\textbf{$\phi$ Directivity}', 'Interpreter', 'latex', 'FontSize', 14);
    end
    grid(ax, 'on');
    if dir_mode == 2, legend(ax, 'Location', 'northeast'); end
end

sgtitle('Comparison NACA 0018 vs 4412: Directivity Patterns', 'FontWeight', 'bold', 'FontSize', 16);