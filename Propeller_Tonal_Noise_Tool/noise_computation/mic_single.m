...
for i = 1:2
    ...
    BPF = omega*prop.B;
    n = 4*prop.B; % Numero armoniche rotore
   ...
   % --- Plot ---
    ...
    for m = 1:2
        ...
        stem(f_norm_analitica, SPL_analitico(m, :), colors{i}, 'filled', 'MarkerSize', 4, 'LineWidth', 1.2, ...
             'DisplayName', [case_labels{i}, ' Tool']);
        
        for k = 1:n
            text(f_norm_analitica(k) + 0.1, SPL_analitico(m, k), ...
                num2str(SPL_analitico(m, k), '%.1f'), ...
                'Color', colors{i}, 'FontSize', 7, 'VerticalAlignment', 'middle');
        end
        
        if i == 1
            if m == 1, plot(f_norm_val, SPL_val_0, 'k-', 'LineWidth', 1.5, 'DisplayName', 'Mid-high fidelity (Reference)');
            else, plot(f_norm_val, SPL_val_90, 'k-', 'LineWidth', 1.5, 'DisplayName', 'Mid-high fidelity (Reference)'); end
        end
...
sgtitle('Far Field Spectra - Comparison NACA 0018 vs 4412', 'FontSize', 14, 'FontWeight', 'bold');